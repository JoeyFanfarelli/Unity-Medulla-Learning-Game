...
function OnTriggerEnter (other: Collider) {
	...
	if (other.tag=="Player"){
		if (PlayerHealth.startMotorChatProg == 0){
			...
			textMesh.text = "Hi friend! Welcome to Cerebellum!";
		}
	}
	...
	if (Input.GetButtonDown("Space")){
		if (PlayerHealth.startMotorStartDialogue==true){
			if(PlayerHealth.startMotorChatProg ==0){
				...
				textMesh.text = "We’re a little wobbly these days, but this was once the happiest place on Medulla!";
				tw.updateText();
			}else if(PlayerHealth.startMotorChatProg==1){
				PlayerHealth.startMotorChatProg =2;
				textMesh.text = "Of course, our spirits aren’t down too much, but we do need some help!";
			}else if(PlayerHealth.startMotorChatProg==2){
				PlayerHealth.startMotorChatProg =3;
				textMesh.text = "I don’t think there’s a person who doesn’t know your name and the things you’ve accomplished.";
				tw.updateText();
			}else if(PlayerHealth.startMotorChatProg==3){
				PlayerHealth.startMotorChatProg =4;
				textMesh.text = "You’re getting close to THOR THE DESTROYER’s territory. Just push a little further.";
				tw.updateText();
			}else if(PlayerHealth.startMotorChatProg==4){
				PlayerHealth.startMotorChatProg = 5;
				textMesh.text = "Before you go, take the <color=red>Cerebellum</color> power and help anybody who is having trouble with their <color=red>balance</color>.";
				tw.updateText();
